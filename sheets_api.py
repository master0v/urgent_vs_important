...
class SheetsClient:
    ...
    def get_all_rows(self, spreadsheet_id: str, sheet_title: str) -> List[List[str]]:
        """Fetch all values without strict header requirement."""
        resp = self.service.spreadsheets().values().get(
            spreadsheetId=spreadsheet_id, range=f"{sheet_title}!A1:H100000"
        ).execute()
    ...
    def get_headers(self, spreadsheet_id: str, sheet_title: str) -> List[str]:
        rows = self.get_all_rows(spreadsheet_id, sheet_title)
        if not rows:
            return list(HEADER)
        h = list(rows[0])
        while len(h) < 8:
    ...
    def read_full_state(
        ...
        """
        Reads the sheet.
        Returns (roots, children_by_parent, flat_tasks)
        flat_tasks includes `_row_index` so we know exactly where it sits in the physical sheet.
        """
        rows = self.get_all_rows(spreadsheet_id, sheet_title)
        if not rows:
            return [], {}, []
        ...
        return roots, children_by_parent, flat_tasks
    ...
    def insert_row(self, spreadsheet_id: str, sheet_title: str, row_index: int, values: List[str]):
        """
        Insert exactly one row at row_index (0-indexed).
            ...
            },
            {
                "updateCells": {
                    ...
                    "rows": [
                        {
                            "values": [
                                {"userEnteredValue": {"stringValue": str(v)}} if str(v).strip() != "" else {"userEnteredValue": {"stringValue": ""}}
                                for v in vals
                            ]
                        }
                    ],
    ...
    def update_row(self, spreadsheet_id: str, sheet_title: str, row_index: int, values: List[str]):
        """
        Update an existing row at row_index.
        """
        sheet_id = self._get_sheet_id(spreadsheet_id, sheet_title)
        vals = list(values)
        while len(vals) < 8:
